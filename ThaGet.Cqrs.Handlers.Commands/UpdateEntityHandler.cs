...
namespace ThaGet.Cqrs.Handlers.Commands
{
    public abstract class UpdateEntityHandler<TCommand, TEntity, TId>
        : CommandEntityHandler<TCommand, byte[], TEntity, TId>
        where TCommand : UpdateEntityCommand<TId, byte[]>
        where TEntity : IEntity<TId>, IVersionedEntity<byte[]>
        where TId : struct
    {
        ...
        protected override async Task<byte[]> Execute(TCommand request, CancellationToken cancellationToken)
        {
            ...
            return MappedCommand.RowVersion;
        }
    }
}
